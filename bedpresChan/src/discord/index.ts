...
import { MessageEmbed, Client, Guild, Collection } from 'discord.js';
...
const shortenDescription = (description: string) => {
    return description.length < 2000 ? description : description.substr(0,2000)+"..."
}
...
const COMMAND_LIST: Record<string, DiscordCommand> = {
    ...
    sources: async (msg, args: string[]) => {
        const list = Object.keys(BEDPRES_SOURCES).map((source) => {
    ...
    here: async (msg, args: string[]) => {
        if(args.length != 2) {
            ...
            if(BEDPRES_SOURCES[subscriptionType] === undefined) {
                ...
                console.log(`Registering channel ${msg.channel.id} for guild ${msg.guild.id}`)
        ...
        if(args.length != 2) {
            ...
            if(args[1] === "event") {
                ...
                for(let event of events) {
                    ...
                    if(event.event_start.getTime() < now.getTime() + 1000 * 60 * 60 * 24 * 5 && 
                ...
                for(let event of events) {
                    //within now and 5 days in the future?
                    if(event.registration_start !== null && 
                        event.registration_start.getTime() < now.getTime() + 1000 * 60 * 60 * 24 * 5 && 
...
client.on('message', async (msg) => {
    ...
    if(msg.author.id != client.user.id) {
        if(msg.content.startsWith(ACTIVATOR)) {
            ...
            if(args.length < 1) {
                msg.reply(ERROR_MESSAGES.INVALID_COMMAND)
            } else {
                if(typeof COMMAND_LIST[args[0]] !== 'undefined') {
                    COMMAND_LIST[args[0]](msg, args);
                } else {
...
client.on('guildCreate', async (guild) => {
    ...
    if(server === undefined) {
        ...
        await server.save()
        guild.systemChannel.send(MESSAGES.GREETING)
    } else {
        console.log("I'm back in a guild i have seen before")
        guild.systemChannel.send(MESSAGES.GREETING_BACK)
    }
...
const broadcastBedpres = async (bedpres: IBedpres, title: string, except?: string[]) => {
    let visitedList: string[] = []
    if(except !== undefined && except.length !== 0) {
        visitedList = visitedList.concat(except)
    }

    await Promise.all(client.guilds.cache.filter((guild: Guild, key: string, collection: Collection<string, Guild>) => {
        ...
        if(server === null) {
            ...
            if(server.subscriptions.has(bedpres.source)) {
                ...
                const channel = await guild.channels.resolve(server.subscriptions.get(bedpres.source))

                await sendBedpres(channel, bedpres, title)
            } else {
                console.log(`Server ${guild.name} doesn't have a subscription for ${bedpres.source}.\n\n${server.subscriptions.keys()}`)
            }
        }
    }))
    await wait(2000)
...
export const sendEventFoundMessage = async (bedpres: IBedpres) => {
    ...
    await broadcastBedpres(bedpres, `:warning: :warning: Ny ${bedpres.type} :warning: :warning:`)
}

export const sendEventChangedMessage = async (bedpres: IBedpres, fields: string[]) => {
    console.log(`Changed bedpres: ${bedpres.title}`)
    await broadcastBedpres(bedpres, `:thinking: Et event har endret seg:  \n${fields.join(", ")}`)
}

export const sendRegistrationReminder = async (bedpres: IBedpres, remindedServers: string[]): Promise<string[]> => {
    console.log(`Registration reminder: ${bedpres.title}. Reminded servers: ${bedpres.remindedServers}`)
    const reminded = await broadcastBedpres(bedpres, `:alarm_clock: :alarm_clock: Husk bedpres-påmelding i dag  ${bedpres.registration_start?.toLocaleTimeString('nb-NO', {timeZone: TIMEZONE})} :alarm_clock: :alarm_clock:`, remindedServers)
    return reminded
}