...
export const processBedpresList = async (list: IBedpres[]) => {
  for (const event of list) {
    ...
    console.log(
      `${event.event_start.getTime()} should be > ${new Date().getTime()}(now)`
    );
    ...
    if (event.event_start.getTime() > new Date().getTime()) {
      ...
      let bedpres = await Bedpres.findOne({
        id: event.id,
        source: event.source,
      });
      ...
      if (bedpres.registration_start !== null) {
        ...
        if (nowDay === attendanceDate) {
          ...
          const remindedList = await sendRegistrationReminder(
            bedpres,
            bedpres.remindedServers
          );
